package com.bangbangcoding.screenmirror.web.favicon
...
import com.bangbangcoding.screenmirror.web.utils.DrawableUtils
...
class FaviconModel @Inject constructor(
    ...
    private val bookmarkIconSize = application.resources.getDimensionPixelSize(R.dimen.padding_3)
    ...
    fun cacheSSForUrl(favicon: Bitmap, url: String): Completable = Completable.create { emitter ->
        val uri = url.toUri().toValidUri() ?: return@create emitter.onComplete()

        logger.log(TAG, "Caching icon for ${uri.host}")
        FileOutputStream(getScreenShotCacheFile(application, uri)).safeUse {
            favicon.compress(Bitmap.CompressFormat.PNG, 100, it)
            it.flush()
            emitter.onComplete()
        }
    ...
    companion object {
        ...
        @WorkerThread
        fun getScreenShotCacheFile(app: Context, validUri: ValidUri): File {
            val hash = validUri.host.hashCode().toString()
            return File(app.cacheDir, "SS_$hash.png")
        }
    }
